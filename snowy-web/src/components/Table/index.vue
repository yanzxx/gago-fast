...
import './index.less'
import tabularDisplayApi from '@/api/biz/tabularDisplay'
...
export default {
	...
	created() {
		...
		const displayColumnMap = userInfo.displayColumnMap
		const displayList = displayColumnMap[pathname]
		if (displayList && displayList.length > 0) {
			this.columnsSetting.forEach((item) => {
				item.checked = !!displayList.includes(item.dataIndex)
			})
		}
	},
	methods: {
		...
		renderHeader() {
			...
			const visibleChange = (visible) => {
				const pathname = window.location.hash
				if (!visible) {
					const displayList = this.columnsSetting
						.map((item) => {
					...
					tabularDisplayApi
						...
						.then(() => {
							...
							const userInfo = tool.data.get('USER_INFO')
							const displayColumnMap = userInfo.displayColumnMap
							displayColumnMap[pathname] = displayList
							userInfo.displayColumnMap = displayColumnMap
