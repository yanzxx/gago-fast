	...
	>
		<a-tabs v-model:activeKey="activeKey" tab-position="left" size="small" @change="paneChange">
			<a-tab-pane v-for="item in iconData" :key="item.key" :tab="item.name">
				...
				<template v-else>
					<div v-for="iconItemIns in item.iconItem" :key="iconItemIns.item">
						<div class="xn-icon-select-list">
							<ul style="display: flex; flex-wrap: wrap">
								<li
									v-for="icon in iconItemIns.item"
									:key="icon"
									:class="icon === modelValue ? 'active' : ''"
									@click="selectIcon('icon-' + icon.font_class)"
									style="display: inline-flex; flex-direction: row; justify-content: center; align-items: center"
								>
									<i class="iconfont" :class="'icon-' + icon.font_class" style="font-size: 24px"></i>
								</li>
							</ul>
						</div>
					</div>
...
<script>
	...
	import jiageConfig from '@/assets/icons/jiage'
	export default {
		data() {
			return {
				visible: false,
		...
		mounted() {
			this.iconData.push(...config.icons, jiageConfig.icons[0])
			console.log('this.iconData', this.iconData)
		},
		methods: {
			// 打开
			showIconModal(value) {
