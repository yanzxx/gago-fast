		...
		>
			...
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						<a @click="immediatelyRun(record)">立即执行</a>
						<a-divider type="vertical" />
						<a v-if="record.jobStatus !== 'STOPPED'" style="cursor: no-drop;color: rgba(0, 0, 0, 0.25);">编辑</a>
						<a @click="form.onOpen(record)" v-else>编辑</a>
						<a-divider type="vertical" />
						<template v-if="record.jobStatus !== 'STOPPED'">
							<a-button type="link" danger size="small" :disabled="record.jobStatus !== 'STOPPED'">删除</a-button>
						</template>
						<template v-else>
