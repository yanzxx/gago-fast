<template>
	...
	>
		<a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical">
			...
			<a-alert style="margin-bottom: 10px" message="温馨提示：新建单页后请在角色管理页面配置该角色的单页权限！" type="warning" closable />
			<a-row :gutter="16">
				<a-col :span="12">
				...
				<a-col :span="12">
					<a-form-item label="是否显示在顶部导航栏:" name="showModuleArea">
						<a-switch v-model:checked="showModuleAreaChecked" checked-children="展示" un-checked-children="隐藏" />
					</a-form-item>
				</a-col>
				<a-col :span="12" v-if="formData.menuType === 'MENU'">
					<a-form-item name="screenFull">
						<template #label>
							<a-tooltip>
								<template #title> 设置全屏时，该菜单的路由必须是一级路由。（即和登录页面是同级路由）否则不生效！ </template>
								<question-circle-outlined />
							</a-tooltip>
							&nbsp 是否全屏展示:：
						</template>
						<a-switch v-model:checked="screenFullChecked" checked-children="是" un-checked-children="否" />
					</a-form-item>
				</a-col>
				<a-col :span="12" v-if="formData.menuType !== 'LINK'">
					<a-form-item label="是否需要隐藏左侧菜单栏:" name="showLeftMenuArea">
						<a-switch v-model:checked="showLeftMenuAreaChecked" checked-children="展示" un-checked-children="隐藏" />
					</a-form-item>
				</a-col>
			</a-row>
		</a-form>
...
<script setup name="spaForm">
	...
	const showModuleAreaChecked = ref(true)
	const screenFullChecked = ref(false)
	const showLeftMenuAreaChecked = ref(true)
	// 打开抽屉
	const onOpen = (record) => {
		visible = true
		formData.value = {
		...
		if (record) {
			...
			if (extJson) {
				...
				const showModuleArea = extJsonObj.showModuleArea
				const screenFull = extJsonObj.screenFull
				const showLeftMenuArea = extJsonObj.showLeftMenuArea
				if (showModuleArea) {
					showModuleAreaChecked.value = showModuleArea
				} else {
					showModuleAreaChecked.value = false
				}
				if (screenFull) {
					screenFullChecked.value = screenFull
				} else {
					screenFullChecked.value = false
				}
				if (showLeftMenuArea) {
					showLeftMenuAreaChecked.value = showLeftMenuArea
				} else {
					showLeftMenuAreaChecked.value = false
				}
			} else {
				showModuleAreaChecked.value = false
				screenFullChecked.value = false
	...
	const onSubmit = () => {
		formRef.value.validate().then(() => {
			...
			param.extJson = JSON.stringify({
				showModuleArea: showModuleAreaChecked.value,
				screenFull: screenFullChecked.value,
				showLeftMenuArea: showLeftMenuAreaChecked.value
			})
