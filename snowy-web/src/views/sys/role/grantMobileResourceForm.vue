	...
	>
		<a-spin :spinning="spinningLoading">
			<a-radio-group v-model:value="moduleId" button-style="solid" style="padding-bottom: 10px">
				<a-radio-button @click="moduleClock('overall')" value="overall"> 全局按钮权限 </a-radio-button>
				<a-radio-button
					:key="module.id"
			...
			<template v-if="moduleId !== 'overall'">
...
<script setup name="grantMobileResourceForm">
	...
	// 获取全局按钮权限
	const getGlobalButton = async () => {
		// 获取表格数据
		spinningLoading.value = true
		const res = await roleApi.mobileGlobalResourceSelector()
		if (res && res.length > 0) {
			const param = {
				id: resultDataModel.id
			}
			// 获取回显数据
			const resEcho = await roleApi.ownMobileGlobalResource(param)
			spinningLoading.value = false
			const grantGlobalList = resEcho && resEcho.grantGlobalList
			res.forEach((item) => {
				if (grantGlobalList.includes(item.id)) {
					item.nameCheck = true
				} else {
	...
	const onSubmit = () => {
		...
		roleApi
			.roleGrantMobileMenu(
				Object.assign({}, param, {
					grantGlobalList: grantGlobalList
				})
			)
			.then(() => {
				onClose()
