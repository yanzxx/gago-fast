			...
			<a-tabs v-model:activeKey="activeTabsKey">
				<a-tab-pane key="1" tab="基础信息" force-render>
					...
					<a-row :gutter="16">
						...
						<a-col :span="6">
							<a-form-item label="选择区划：" name="regionCode">
								<a-tree-select
									...
									:tree-default-expanded-keys="regionTreeDefaultExpandedKeys"
									:field-names="{
										children: 'children',
										label: 'name',
										value: 'id'
...
<script setup>
	...
	const regionTreeDefaultExpandedKeys = ref([])
	// 主职职位数据
		...
		nextTick(() => {
			...
			regionApi.regionTree().then((res) => {
				if (res !== null) {
					...
					// 默认展开2级
					regionTreeData.value.forEach((item) => {
						// 因为0的顶级
						if (item.parentId === '0') {
							regionTreeDefaultExpandedKeys.value.push(item.id)
							// 取到下级ID
							if (item.children) {
								item.children.forEach((items) => {
									regionTreeDefaultExpandedKeys.value.push(items.id)
								})
							}
						}
					})
	...
	const onClose = () => {
		...
		regionTreeDefaultExpandedKeys.value = []
		positionData.value = []
		directorData.value = []
		visible = false
	}
	...
	const convertFormData = (record) => {
		...
		userApi.userDetail(param).then((data) => {
			...
			formData.value.entryDate = data.entryDate ? dayjs(data.entryDate) : undefined
	...
	/**
	 * 设置组织树展开节点
	 * @param {Array} data - 组织树数据
	 */
	const setOrgTreeExpandedKeys = (data) => {
		treeDefaultExpandedKeys.value = []
		data.forEach((item) => {
			if (item.parentId === '0') {
	 ...
	 * 加载所有组织数据
	 */
	const loadAllOrgData = () => {
		userApi.userOrgTreeSelector().then((res) => {
			if (res !== null) {
				treeData.value = res
				setOrgTreeExpandedKeys(res)
			}
		})
	}
	// 附属职位信息增行
