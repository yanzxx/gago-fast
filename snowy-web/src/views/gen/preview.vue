...
<script setup name="genPreview">
	...
	const onOpen = (record) => {
		...
		genBasicApi.getGenFileNames().then((res) => {
			const genFileNames = res
			const param = {
				id: record.id,
				genBackendFileNames: genFileNames.genBackendFileNames.join(','),
				genFrontFileNames: genFileNames.genFrontFileNames.join(','),
				genSqlFileNames: genFileNames.genSqlFileNames.join(',')
			}
			genBasicApi
