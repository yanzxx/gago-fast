	...
	<div :style="{ width: record.options.width }">
		...
		>
			<a-button v-if="fileList.length < record.options.limit" :disabled="record.options.disabled || parentDisabled">
				<upload-outlined />{{ record.options.placeholder }}
			</a-button>
		</a-upload>
		<a-upload-dragger
			v-else
		...
		methods: {
			...
			handlePreview(file) {
				...
				if (downloadWay === 'a') {
					...
					a.href = file.response ? file.response.data : file.url //file.response.data || file.thumbUrl;
					a.download = file.name
					a.click()
				} else if (downloadWay === 'ajax') {
					// 使用ajax获取文件blob，并保持到本地
					this.getBlob(file.response.data || file.thumbUrl).then((blob) => {
