...
<template>
	<div class="form-panel">
		...
		<a-form
			class="a-form-box snowy-form-build"
			:model="form"
	...
	export default {
		...
		methods: {
			deepClone(evt) {
				...
				// json深拷贝一次
				const list = cloneDeep(this.data.list)
				// list[newIndex].key = list[newIndex].type + "_" + Date.now();
				this.data.list = list
				// 删除icon及compoent属性
			...
			handleColAdd(evt, columns, isCopy = false, parentRecord) {
				// 重置或者生成key值
				const newIndex = evt.newIndex
				...
				this.$emit('handleSetSelectItem', columns[newIndex], parentRecord)
			},
			dragStart(evt, list) {
				// 拖拽结束,自动选择拖拽的控件项
				this.$emit('handleSetSelectItem', list[evt.oldIndex])
			},
			handleSelectItem(record, parentRecord) {
				// 修改选择Item
				this.$emit('handleSetSelectItem', record, parentRecord)
			},
