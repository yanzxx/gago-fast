	...
	<a-config-provider :locale="locale">
		<div class="form-designer-container-9136076486841527">
			<div style="margin: 20px;">
				<label style="font-weight: 900;">表单设计器</label>
			</div>
			<!-- 操作区域 start -->
			<operatingArea
				v-if="toolbarsTop"
			...
			>
				<!-- 左侧控件区域 start -->
				<aside class="left">
					<a-collapse :default-active-key="collapseDefaultActiveKey" @change="collapseChange">
						<!-- 基础控件 start -->
						<a-collapse-panel v-if="basicsArray.length > 0" key="1" header="基础控件">
							<collapseItem
								:list="basicsArray"
						...
						<!-- 基础控件 end -->
						<!-- 高级控件 start -->
						<a-collapse-panel v-if="highArray.length > 0" key="2" header="高级控件">
							<collapseItem
								:list="highArray"
								@generateKey="generateKey"
								@handleListPush="handleListPush"
						...
						<!-- 高级控件 end -->
						<!-- 自定义控件 start -->
						<a-collapse-panel v-if="customComponents.list.length > 0" key="3" :header="customComponents.title">
							<collapseItem
						...
						<!-- 布局控件 start -->
						<a-collapse-panel v-if="layoutArray.length > 0" key="4" header="布局控件">
							<collapseItem
								:list="layoutArray"
								@generateKey="generateKey"
								@handleListPush="handleListPush"
				...
				<!-- 右侧控件属性区域 start -->
				<aside class="right">
					<a-tabs :active-key="activeKey" centered @change="changeTab">
...
<script>
	...
	import { basicsList, layoutList, customComponents, highList } from './config/formItemsConfig'
	...
	export default {
		...
		computed: {
			...
			highArray() {
				// 计算需要显示的基础字段
				return highList.filter((item) => this.fields.includes(item.type))
			},
			layoutArray() {
				// 计算需要显示的布局字段
			...
			collapseDefaultActiveKey() {
				...
				if (config.storageType === 'localStorage') {
					defaultActiveKey = window.localStorage.getItem(config.STORAGE_PREFIX + 'collapseDefaultActiveKey')
				} else {
					defaultActiveKey = window.sessionStorage.getItem(config.STORAGE_PREFIX + 'collapseDefaultActiveKey')
				}
				if (defaultActiveKey) {
		...
		methods: {
			...
			handleSetSelectItem(record, parentRecord) {
				// 操作间隔不能低于100毫秒
			...
			collapseChange(val) {
				...
				if (config.storageType === 'localStorage') {
					window.localStorage.setItem(config.STORAGE_PREFIX + 'collapseDefaultActiveKey', val)
				} else {
					window.sessionStorage.setItem(config.STORAGE_PREFIX + 'collapseDefaultActiveKey', val)
				}
			},
			handleStart(type) {
