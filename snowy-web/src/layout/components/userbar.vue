<template>
	<div class="user-bar">
		<div class="branches panel-item hidden-sm-and-down" @click="openDoc" title="开发文档">
			<copy-outlined style="font-size: 18px; color: #ffffff" />
		</div>
		<div class="branches panel-item hidden-sm-and-down" @click="openThirdPartyUiLibrary" title="UI库链接">
			<branches-outlined style="font-size: 18px; color: #ffffff" />
		</div>
		<div class="branches panel-item hidden-sm-and-down" @click="openMobilePath" title="移动端链接">
			<android-outlined style="font-size: 18px; color: #ffffff" />
		</div>
		<div v-if="!ismobile" class="search panel-item hidden-sm-and-down" @click="handleSearchClick" title="搜索功能">
			<search-outlined style="font-size: 18px; color: #ffffff" />
		</div>
		<div class="screen panel-item hidden-sm-and-down" @click="fullscreen" title="全屏">
			<fullscreen-outlined style="font-size: 18px; color: #ffffff" />
		</div>
		<!--<devUserMessage />-->
		<a-dropdown class="user panel-item">
			...
			<template #overlay>
				<a-menu>
					<a-menu-item key="uc" @click="handleUser('uc')" v-if="userCenterShow">
					...
					<a-menu-divider />
					<a-menu-item key="outLogin" @click="handleUser('clearCache')">
		...
		<a-dropdown v-if="!ismobile" class="panel-item">
			<global-outlined style="font-size: 18px; color: #ffffff" />
			<template #overlay>
		...
		<div v-if="setDeawer === 'true'" class="setting panel-item" @click="openSetting">
			<layout-outlined style="font-size: 18px; color: #ffffff" />
		</div>
	</div>

	<!-- 整体风格设置抽屉 -->
	<a-drawer v-model:visible="settingDialog" :closable="false" width="300">
		<setting style="font-size: 18px; color: #ffffff" />
	</a-drawer>
	<!-- 搜索面板 -->
	<xn-form-container
		title="搜索"
	...
	export default {
		...
		data() {
			return {
				...
				pathOfMobile: 'http://39.98.43.185:31715/#/login',
				officialDocument: 'https://gago.yuque.com/nxwayb/ofd1al',
				userCenterShow: true
			}
		},
		computed: {
		...
		created() {
			...
			const spa = tool.data.get('SPA')
			if (spa && spa.length > 0) {
				const names = spa.map((item) => {
					return item.name
				})
				if (names.includes('userCenter')) {
		...
		methods: {
			...
			// 打开文档
			openDoc() {
				window.open(this.officialDocument)
			},
			// 打开移动端
			openMobilePath() {
