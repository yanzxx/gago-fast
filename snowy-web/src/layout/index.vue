<template>
	<!-- 经典布局 -->
	<template v-if="layout === 'doublerow'">
		<a-layout>
			<span>
				...
				<a-layout-sider
					...
					width="220"
				>
					<div :class="menuIsCollapse ? 'aminui-side isCollapse' : 'aminui-side'" id="sliderLeftEle">
			...
			<a-layout>
				<div id="snowyHeader" class="snowy-header">
					<div class="snowy-header-left" style="padding-left: 0px">
						...
						>
							<MenuUnfoldOutlined v-if="menuIsCollapse" style="font-size: 18px; color: #ffffff" />
							<MenuFoldOutlined v-else style="font-size: 18px; color: #ffffff" />
						</div>
						<moduleMenu
							@switchModule="switchModule"
							@switchModuleWithNoChangePage="switchModuleWithNoChangePage"
				...
				<a-layout-content class="main-content-wrapper">
					<div id="adminui-main" class="adminui-main">
						<Topbar v-if="!ismobile && breadcrumbOpen" :class="layoutTagsOpen ? 'mgB10' : ''" />
						<Tags v-if="!ismobile && layoutTagsOpen" />
						<div class="lineBetween" v-if="!layoutTagsOpen && breadcrumbOpen">
	...
	<!-- 双排菜单布局 -->
	<template v-else-if="layout === 'classical'">
		<a-layout>
			...
			<a-layout>
				<div id="snowyHeader" class="snowy-header">
					<div class="snowy-header-left" style="padding-left: 0px">
						...
						<moduleMenu @switchModule="switchModule" @switchModuleWithNoChangePage="switchModuleWithNoChangePage" />
...
<script>
	...
	export default defineComponent({
		...
		methods: {
			...
			// 切换应用，但是按照路由显示，不跳转
			switchModuleWithNoChangePage(id) {
				const menu = this.moduleMenu
				if (menu.length > 0) {
					const menus = menu.filter((item) => item.id === id)[0].children
					if (menus.length > 0) {
				...
				map &&
					map.forEach((item) => {
						...
						if (item.category === 'SPA') {
							if (names.includes(item.name) && item.name === 'index') {
			...
			switchoverTopHanderThemeColor() {
				...
				// 设置侧边栏颜色
				/*const sliderLeftEle = document.getElementById('sliderLeftEle')
				this.topHanderThemeColorOpen
					? sliderLeftEle.classList.add('snowy-header-primary-color')
					: sliderLeftEle.classList.remove('snowy-header-primary-color')*/
				// 如果是双排菜单，吧第二排的也给渲染了
				if (this.layout === 'doublerow') {
