...
public abstract class SaBaseLoginUser {
    ...
    /** 角色集合 */
    @ApiModelProperty(value = "角色集合", position = 54, hidden = true)
    private List<JSONObject> roles;

    /** 数据范围集合 */
    @ApiModelProperty(value = "数据范围集合", position = 55, hidden = true)
    private List<DataScope> dataScopeList;

    /** 列表不显示列集合 */
    @ApiModelProperty(value = "列表显示列集合", position = 55, hidden = true)
    private Map<String, List<String>> displayColumnMap;

    /** 用户密码hash值 */
    @ApiModelProperty(value = "用户密码hash值", position = 56)
