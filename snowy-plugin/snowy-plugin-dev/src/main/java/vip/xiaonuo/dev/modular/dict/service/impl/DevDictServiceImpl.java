...
public class DevDictServiceImpl extends ServiceImpl<DevDictMapper, DevDict> implements DevDictService, InitializingBean {
    ...
    private void refreshTransCache() {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            Map<String,List<DevDict>> devDictGroupByPIDMap = devDictList
                    .stream()
                    .filter(dict -> !ROOT_PARENT_ID.equals(dict.getParentId()))
                    .collect(Collectors.groupingBy(DevDict::getParentId));

            Map<String,String> parentDictIdValMap = devDictList
                    .stream()
                    .filter(dict -> ROOT_PARENT_ID.equals(dict.getParentId()))
            ...
            for (String parentId : parentDictIdValMap.keySet()) {
                if(devDictGroupByPIDMap.containsKey(parentId)){
                    dictionaryTransService.refreshCache(parentDictIdValMap.get(parentId), devDictGroupByPIDMap.get(parentId).stream().collect(Collectors.toMap(DevDict::getDictValue, DevDict::getDictLabel)));
                }
            }
            return null;
        });
