...
public class DevJobServiceImpl extends ServiceImpl<DevJobMapper, DevJob> implements DevJobService {
    ...
    public Page<DevJob> page(DevJobPageParam devJobPageParam) {
        ...
        Page<DevJob> page = this.page(CommonPageRequest.defaultPage(), queryWrapper);
        for (DevJob record : page.getRecords()) {
            if (!record.ifRunning()) {
                record.setNextTime("任务处于关闭状态");
                continue;
            }
            ...
            Date start = new Date();
            Date end = DateUtil.offsetMonth(start, 12);
            List<Date> dates = CronPatternUtil.matchedDates(cronExpression, start, end, 1, true);
            record.setNextTime(dates.isEmpty() ? null : DateUtil.formatDateTime(dates.get(0)));
        }
