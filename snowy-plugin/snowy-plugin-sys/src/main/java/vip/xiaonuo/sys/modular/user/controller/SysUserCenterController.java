...
import vip.xiaonuo.sys.modular.user.result.SysUserPasswordExpireResult;
...
public class SysUserCenterController {
    ...
    @ApiOperation("获取角色拥有的单页")
    @GetMapping("/sys/userCenter/ownSpa")
    public CommonResult<List<SysSpa>> ownSpa(@Valid SysRoleIdParam sysRoleIdParam) {
        return CommonResult.data(sysUserService.ownSpa(sysRoleIdParam));
    }
     ...
     *
     * @author xuyuxiang
     * @date 2025/5/16 19:30
     */
    @ApiOperationSupport(order = 19)
    @ApiOperation("检查用户密码是否过期")
    @GetMapping("/sys/userCenter/checkPasswordExpire")
    public CommonResult<SysUserPasswordExpireResult> checkPasswordExpire() {
        return CommonResult.data(sysUserService.checkPasswordExpire());
    }
}
