...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
    ...
    @Resource
    private MobileGlobalResourceApi mobileGlobalResourceApi;
    ...
    @Override
    public void grantSpa(SysRoleGrantSpaParam sysRoleGrantSpaParam) {
        String id = sysRoleGrantSpaParam.getId();
        List<String> spaIdList = sysRoleGrantSpaParam.getGrantInfoList().stream()
                .map(SysRoleGrantSpaParam.SysRoleGrantSpa::getSpaId).collect(Collectors.toList());

        List<String> extJsonList = sysRoleGrantSpaParam.getGrantInfoList().stream()
                .map(JSONUtil::toJsonStr).collect(Collectors.toList());
        sysRelationService.saveRelationBatchWithClear(id, spaIdList, SysRelationCategoryEnum.SYS_ROLE_HAS_SPA.getValue(),
                extJsonList);
    }

    @Override
    public void grantTerminal(SysRoleGrantTerminalParam sysRoleGrantTerminalParam) {
        String roleId = sysRoleGrantTerminalParam.getId();
        List<String> grantTerminalList = sysRoleGrantTerminalParam.getGrantTerminalList();

        sysRelationService.saveRelationBatchWithClear(roleId, grantTerminalList, SysRelationCategoryEnum.SYS_ROLE_HAS_TERMINAL.getValue());
    }
    ...
    public SysRoleOwnMobileMenuResult ownMobileMenu(SysRoleIdParam sysRoleIdParam) {
        List<SysRoleOwnMobileMenuResult.SysRoleOwnMobileMenu> grantInfoList =
                sysRelationService.getRelationListByObjectIdAndCategory(sysRoleIdParam.getId(), SysRelationCategoryEnum.SYS_ROLE_HAS_MOBILE_MENU.getValue())
                        .stream()
                        .map(sysRelation -> JSONUtil.toBean(sysRelation.getExtJson(), SysRoleOwnMobileMenuResult.SysRoleOwnMobileMenu.class))
        ...
        sysRoleOwnMobileMenuResult.setGrantInfoList(grantInfoList);
        return sysRoleOwnMobileMenuResult;
    }

    @Override
    public SysRoleOwnMobileGlobalResourceTreeResult ownMobileGlobalResource(SysRoleIdParam sysRoleIdParam) {
        List<String> grantGlobalList = sysRelationService.getRelationListByObjectIdAndCategory(sysRoleIdParam.getId(), SysRelationCategoryEnum.SYS_ROLE_HAS_MOBILE_GLOBAL_RESOURCE.getValue())
                .stream()
                .map(SysRelation::getTargetId)
                .collect(Collectors.toList());

        SysRoleOwnMobileGlobalResourceTreeResult sysRoleOwnMobileGlobalResourceTreeResult = new SysRoleOwnMobileGlobalResourceTreeResult();
        sysRoleOwnMobileGlobalResourceTreeResult.setId(sysRoleIdParam.getId());
        sysRoleOwnMobileGlobalResourceTreeResult.setGrantGlobalList(grantGlobalList);
        return sysRoleOwnMobileGlobalResourceTreeResult;
    }

    @Override
    public SysRoleOwnTerminalResult ownTerminal(SysRoleIdParam sysRoleIdParam) {
        List<String> grantInfoList = sysRelationService.getRelationListByObjectIdAndCategory(sysRoleIdParam.getId(), SysRelationCategoryEnum.SYS_ROLE_HAS_TERMINAL.getValue())
                .stream()
                .map(SysRelation::getTargetId)
                .collect(Collectors.toList());

        SysRoleOwnTerminalResult sysRoleOwnTerminalResult = new SysRoleOwnTerminalResult();
        sysRoleOwnTerminalResult.setId(sysRoleIdParam.getId());
        sysRoleOwnTerminalResult.setGrantInfoList(grantInfoList);
        return sysRoleOwnTerminalResult;
    }
    ...
    public void grantMobileMenu(SysRoleGrantMobileMenuParam sysRoleGrantMobileMenuParam) {
        ...
        List<String> menuIdList = sysRoleGrantMobileMenuParam.getGrantInfoList().stream().map(SysRoleGrantMobileMenuParam.SysRoleGrantMobileMenu::getMenuId).collect(Collectors.toList());

        List<String> extJsonList = sysRoleGrantMobileMenuParam.getGrantInfoList().stream().map(JSONUtil::toJsonStr).collect(Collectors.toList());

        sysRelationService.saveRelationBatchWithClear(id, menuIdList, SysRelationCategoryEnum.SYS_ROLE_HAS_MOBILE_MENU.getValue(), extJsonList);

        List<String> grantGlobalList = sysRoleGrantMobileMenuParam.getGrantGlobalList();

        sysRelationService.saveRelationBatchWithClear(id, grantGlobalList, SysRelationCategoryEnum.SYS_ROLE_HAS_MOBILE_GLOBAL_RESOURCE.getValue());
    ...
    @Override
    public List<SysRoleGrantSpaResult> spaTreeSelector() {
        // 查询出所有的单页面
        LambdaQueryWrapper<SysMenu> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.in(SysMenu::getCategory, SysResourceCategoryEnum.SPA.getValue(), SysResourceCategoryEnum.BUTTON.getValue());
        List<SysMenu> allMenuAndButtonAndFieldList = sysMenuService.list(lambdaQueryWrapper);

        List<SysSpa> sysSpaList = CollectionUtil.newArrayList();
        List<SysMenu> sysButtonList = CollectionUtil.newArrayList();

        allMenuAndButtonAndFieldList.forEach(sysMenu -> {
            if (sysMenu.getCategory().equals(SysResourceCategoryEnum.SPA.getValue())) {
                sysSpaList.add(BeanUtil.copyProperties(sysMenu, SysSpa.class));
            }
            if (sysMenu.getCategory().equals(SysResourceCategoryEnum.BUTTON.getValue())) {
                sysButtonList.add(sysMenu);
            }
        ...
        return sysSpaList.stream().map(sysSpa -> {
            String id = sysSpa.getId();
            List<SysMenu> buttons = this.getChildListById(sysButtonList, id, false);

            SysRoleGrantSpaResult sysRoleGrantSpaResult = new SysRoleGrantSpaResult();
            sysRoleGrantSpaResult.setId(id);
            sysRoleGrantSpaResult.setTitle(sysSpa.getTitle());
            sysRoleGrantSpaResult.setButton(BeanUtil.copyToList(buttons, SysRoleGrantSpaResult.SysRoleGrantResourceButtonResult.class));

            return sysRoleGrantSpaResult;
    ...
    @Override
    public List<SysRoleGrantTerminalResult> terminalSelector() {
        return BeanUtil.copyToList(saBaseLoginUserApi.getLoginTerminal(), SysRoleGrantTerminalResult.class);
    }
    ...
    @Override
    public List<SysRoleGrantMobileGlobalResourceTreeResult> mobileGlobalResourceSelector() {
        return BeanUtil.copyToList(mobileGlobalResourceApi.mobileGlobalResourceSelector(), SysRoleGrantMobileGlobalResourceTreeResult.class);
    }
