...
public class SysButtonServiceImpl extends ServiceImpl<SysButtonMapper, SysButton> implements SysButtonService {
    ...
    public void addForGenButton(String menuId, String className, String functionName) {
        ...
        CollectionUtil.newArrayList(
                ...
                JSONUtil.createObj().set("title", "批量删除"+ functionName).set("code", classNameFirstLower + "BatchDelete").set("sortCode", 4),
                JSONUtil.createObj().set("title", "Excel导入" + functionName).set("code", classNameFirstLower + "ExcelIn").set("sortCode", 5),
                JSONUtil.createObj().set("title", "Excel导出" + functionName).set("code", classNameFirstLower + "ExcelOut").set("sortCode", 6))
                .forEach(jsonObject -> {
