...
import vip.xiaonuo.sys.modular.role.param.SysRoleIdParam;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public void resetPassword(SysUserIdParam sysUserIdParam) {
        this.update(new LambdaUpdateWrapper<SysUser>().eq(SysUser::getId,
                ...
                .set(SysUser::getPasswordUpdateTime, new Date()));
    }
    ...
    public List<Tree<String>> ownMenu(SysUserIdParam sysUserIdParam) {
        ...
        List<JSONObject> resultJsonObjectList = resultList.stream().map(sysMenu -> {
            ...
            menuJsonObject.set("extJson", sysMenu.getExtJson());
    ...
    @Override
    public List<SysSpa> ownSpa(SysRoleIdParam sysRoleIdParam) {
        List<String> spaIdList = sysRelationService.getRelationListByObjectIdListAndCategory(Arrays.asList(sysRoleIdParam.getId().split(",")), SysRelationCategoryEnum.SYS_ROLE_HAS_SPA.getValue())
                .stream().map(SysRelation::getTargetId).collect(Collectors.toList());


        return CollectionUtil.isEmpty(spaIdList) ? new ArrayList<>() : sysSpaService.listByIds(spaIdList);
    ...
    public List<String> getButtonCodeList(List<String> userAndRoleIdList) {
        ...
        LambdaQueryWrapper<SysRelation> lambdaQueryWrapper = new LambdaQueryWrapper<SysRelation>()
                .in(SysRelation::getObjectId, userAndRoleIdList)
                .in(SysRelation::getCategory, SysRelationCategoryEnum.SYS_USER_HAS_RESOURCE.getValue(), SysRelationCategoryEnum.SYS_ROLE_HAS_RESOURCE.getValue());

        sysRelationService.list(lambdaQueryWrapper)
                .forEach(sysRelation -> {
    ...
    public List<String> getMobileButtonCodeList(List<String> userAndRoleIdList) {
        List<String> buttonIdList = CollectionUtil.newArrayList();
        sysRelationService.getRelationListByObjectIdListAndCategory(userAndRoleIdList, SysRelationCategoryEnum.SYS_ROLE_HAS_MOBILE_MENU.getValue())
                .forEach(sysRelation -> {
                    if (ObjectUtil.isNotEmpty(sysRelation.getExtJson())) {
                        buttonIdList.addAll(JSONUtil.parseObj(sysRelation.getExtJson()).getBeanList("buttonInfo", String.class));
                    }
        ...
        return CollectionUtil.isEmpty(buttonIdList) ?  CollectionUtil.newArrayList() :  mobileButtonApi.getCodeListByIds(buttonIdList);

    }

    @Override
    public List<String> getMobileGlobalCodeList(List<String> userAndRoleIdList) {
        List<String> mobileGlobalResourceIds = sysRelationService.getRelationListByObjectIdListAndCategory(userAndRoleIdList, SysRelationCategoryEnum.SYS_ROLE_HAS_MOBILE_GLOBAL_RESOURCE.getValue())
                .stream()
                .map(SysRelation::getTargetId)
                .collect(Collectors.toList());

        return CollectionUtil.isEmpty(mobileGlobalResourceIds) ?  CollectionUtil.newArrayList() : mobileGlobalResourceApi.getCodeListByIds(mobileGlobalResourceIds);
    }
    ...
    public List<JSONObject> getPermissionList(List<String> userAndRoleIdList, String orgId) {
        Map<String, List<SysRelation>> map = sysRelationService.list(new LambdaQueryWrapper<SysRelation>()
                        .in(SysRelation::getObjectId, userAndRoleIdList)
                        .in(SysRelation::getCategory, SysRelationCategoryEnum.SYS_USER_HAS_PERMISSION.getValue(), SysRelationCategoryEnum.SYS_ROLE_HAS_PERMISSION.getValue()))
                        .stream()
    ...
    @Override
    public List<SysUser> getAllUserSelectorList() {
        return this.list(new LambdaQueryWrapper<SysUser>().select(SysUser::getId, SysUser::getOrgId,
    ...
    public List<String> getDeviceByRoleIdList(List<String> roleIdList, String device) {
        ...
        List<SysRelation> sysRelations = sysRelationService.getRelationListByObjectIdListAndCategory(roleIdList, SysRelationCategoryEnum.SYS_ROLE_HAS_TERMINAL.getValue());
        return sysRelations.stream().filter(sysRelation -> Objects.equals(device, sysRelation.getTargetId())).distinct().map(SysRelation::getObjectId).collect(Collectors.toList());
    }

    @Override
    public SysUserPasswordExpireResult checkPasswordExpire() {
        SysUserPasswordExpireResult result = new SysUserPasswordExpireResult();
        
        // 获取当前登录用户
        SysUser sysUser = this.queryEntity(StpUtil.getLoginIdAsString());
        
        // 获取密码过期配置
        String passwordExpireEnabled = devConfigApi.getValueByKey("SNOWY_SYS_PASSWORD_EXPIRE_ENABLED");
        
        // 如果功能未开启，直接返回未过期
        if(!"true".equals(passwordExpireEnabled)) {
            result.setExpired(false);
            return result;
        }
        
        // 获取密码过期时长(分钟)
        String passwordExpireMinutesStr = devConfigApi.getValueByKey("SNOWY_SYS_PASSWORD_EXPIRE_MINUTES");
        
        // 如果密码过期时长配置不存在，直接返回未过期
        if(ObjectUtil.isEmpty(passwordExpireMinutesStr)) {
            result.setExpired(false);
            return result;
        }
        
        // 计算密码是否过期
        Date passwordUpdateTime = sysUser.getPasswordUpdateTime();
        ...
        // 计算密码距今经过的分钟数
        long passedMillis = System.currentTimeMillis() - passwordUpdateTime.getTime();
        long passedMinutes = passedMillis / (60 * 1000);
        
        // 判断是否过期
        int passwordExpireMinutes = Integer.parseInt(passwordExpireMinutesStr);
        boolean isExpired = passedMinutes >= passwordExpireMinutes;
        result.setExpired(isExpired);
        
        // 格式化消息
        String timeDesc;
        if(passedMinutes < 60) {
            timeDesc = passedMinutes + "分钟";
        } else if(passedMinutes < 24 * 60) {
            timeDesc = (passedMinutes / 60) + "小时";
        } else {
            timeDesc = (passedMinutes / (24 * 60)) + "天";
        }
        
        result.setMessage("您的密码已经" + timeDesc + "未更新，为保证账号安全，请及时修改密码。");
